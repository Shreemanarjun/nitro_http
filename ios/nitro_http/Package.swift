...
// `<app>/ios/Flutter/ephemeral/Packages/.packages/nitro_http` and points the
...
if hasVendoredCurl {
    ...
    // No `curl` fallback on iOS, unlike the macOS manifest: the iPhoneOS SDK
    // ships neither curl headers nor a libcurl stub, so a vendored slice is
    // mandatory here. Resolution still succeeds without one so a Dart-only
    // consumer is not blocked; the build then fails on undefined `curl_*`
    // symbols, and the podspec's fetch script explains how to supply a slice.
}
