...
Pod::Spec.new do |s|
  ...
  s.description      = <<-DESC
The macOS half of nitro_http: a C++ HTTP engine built on libcurl and driven over
...
overrides and DoH, cookies, a disk cache, and WebSockets.
  ...
  #
  # Without a vendored slice, link the platform libcurl the Apple SDK ships
  # (headers at $(SDKROOT)/usr/include/curl, libcurl.tbd alongside). That build
  # loses HTTP/3 — which the engine still reports honestly at runtime through
  # `curl_version_info` — plus brotli and zstd, which are compile-time (see
  # GCC_PREPROCESSOR_DEFINITIONS below). It builds and runs today, which is
  # worth far more than failing to link until a deps release exists.
  s.libraries = have_vendored_curl ? ['z', 'resolv'] : ['z', 'resolv', 'curl']
